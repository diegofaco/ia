...
NUM_OPTIONS = ["0"] + [str(i) for i in range(1, 100)]
...
TILE_OPTIONS = ["--tile", ""]
WEIRD_OPTIONS = ["--weird " + str(i) for i in [0, 10, 25, 50, 75, 100, 150, 250, 350, 500, 750, 1000, 1250, 1500, 2000, 2500, 3000]]
...
class MainWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.file_selector = self.create_combobox(NUM_OPTIONS)
        self.category_selectors = {category: (self.create_combobox(NUM_OPTIONS), QCheckBox("Range?")) for category in self.categories.keys()}
        self.aspect_ratio_selector = self.create_combobox(ASPECT_RATIO_OPTIONS, "--ar 16:9")
        self.chaos_selector = self.create_combobox(CHAOS_OPTIONS, "--c 0")
        self.stylize_selector = self.create_combobox(STYLIZE_OPTIONS, "--s 100")
        self.tile_selector = self.create_combobox(TILE_OPTIONS, "")
        self.weird_selector = self.create_combobox(WEIRD_OPTIONS, "--weird 0")
